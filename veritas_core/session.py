...
class SessionManager:
    ...
    def get_data(self, key: str) -> pd.DataFrame: return st.session_state.get(f"{key}_data", pd.DataFrame())
    def get_page_state(self, key: str, default: Any = None) -> Any: return st.session_state.page_states.get(key, default)
    def update_page_state(self, key: str, value: Any): st.session_state.page_states[key] = value
    def clear_page_state(self, key: str):
        if key in st.session_state.page_states: del st.session_state.page_states[key]

    # ... (all business logic methods like get_kpi, generate_draft_report, etc. are correct and remain here)
    ...
    def generate_draft_report(self, **kwargs):
        ...
        self.update_page_state('draft_report', {'filename': filename, 'mime': mime, 'watermarked_bytes': bytes_, 'report_data': report_data})
    ...
    def get_risk_matrix_data(self) -> pd.DataFrame:
        return pd.DataFrame({"program_id": ["VX-561", "VX-121", "VX-809", "VX-984"], "days_to_milestone": [50, 80, 200, 150], "dqs": [92, 98, 99, 96], "active_deviations": [8, 2, 1, 4], "risk_quadrant": ["High Priority", "On Track", "On Track", "Data Risk"]})
