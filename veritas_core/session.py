...
def initialize_session():
    ...
    other page can access them. This resolves all race conditions.
    """
    # Use a single key to check if the entire initialization has been completed.
    if 'veritas_initialized' in st.session_state:
        return

    # 1. Initialize backend components first
    st.session_state.settings = settings
    # In a real app, an environment variable would switch this to ProdDataRepository
    st.session_state.repo = repository.MockDataRepository()
    
    # 2. Load all data into the session state using the repository
    ...
    # 3. Initialize containers for page-specific states and auth
    st.session_state.page_states = {}
    auth.initialize_auth_state()

    # 4. Write initial login audit log
...
class SessionManager:
    ...
    def generate_draft_report(self, **kwargs):
        ...
        report_data['cqa'] = cqa
        report_data['data'] = report_data['report_df'] # FIX FOR KeyError: 'data'
    ...
    def get_kpi(self, kpi_name: str) -> Dict:
        if kpi_name == 'active_deviations':
            ...
            return {'value': len(df[df['status'] != 'Closed']), 'sme_info': "Total open quality events. A direct measure of the current problem-solving burden."}
